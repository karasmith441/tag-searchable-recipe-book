...
function clearSearch(baseUrl, dataJSON)
{
	inpt.value = "";
...
function readBase(dataJSON)
{
	sessionStorage.setItem('search', inpt.value);
	fetch(dataJSON).then((response) => response.json()).then((json) => generateLinks(json));
}
...
function generateLinks(dataJSON)
{
